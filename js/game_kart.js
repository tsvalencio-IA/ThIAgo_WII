...
// ENGINEER: CODE 177 + RANKING & MAP FIX
// =====================================================
...
buildTrack: function() {
    ...
    buildMiniMap(segments);
        ...
        updatePhysics: function(w, h, pose) {
            ...
            } else { d.inputState = 0; d.targetSteer = 0; d.virtualWheel.opacity *= 0.9; }
            ...
            d.rivals.forEach(r => {
                ...
                r.x += ((-(segments[Math.floor(r.pos/SEGMENT_LENGTH)%segments.length].curve * 0.6)) - r.x) * 0.05;

                let playerTotalDist = d.pos + (d.lap * trackLength);
                let rivalTotalDist = r.pos + ((d.lap - (dist > trackLength/2 ? 0 : 1)) * trackLength);
            ...
            if (d.state === 'finished' && d.speed > 0.1) {
                d.speed *= 0.95;
            ...
            for(let n = 99; n >= 0; n--) {
                ...
                seg.obs.forEach(o => {
                    ...
                    if (o.type === 'cone') {
                        ...
                        ctx.fillStyle = '#000'; ctx.textAlign='center'; ctx.font = `bold ${size*0.4}px Arial`; ctx.fillText(seg.curve > 0 ? ">>>" : "<<<", sX, coord.y - size*0.2);
                    }
                });
            }
        ...
        renderUI: function(ctx, w, h) {
            ...
            if (d.state === 'race') {
                ...
                if (minimapPoints.length > 1) {
                    ...
                    ctx.beginPath(); ctx.arc(minimapPoints[pi].x, minimapPoints[pi].y, 6, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
                    d.rivals.forEach(r => {
                ...
                if (d.virtualWheel.opacity > 0.01) {
                    ...
                    ctx.rotate(d.steer * 1.4); ctx.fillStyle = '#ff3300'; ctx.fillRect(-4, -vw.r + 10, 8, 22);
